...
void CUVGLWidget::setAspectRatio(double ratio) {
	aspect_ratio = ratio;
	if (ABS(ratio) < 1e-6) {
		setVertices(1.0);
	} else {
...
void CUVGLWidget::drawFrame(CUVFrame* pFrame) {
...
void CUVGLWidget::drawTexture(CUVRect rc, GLTexture* tex) {
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
...
void CUVGLWidget::drawRect(CUVRect rc, CUVColor clr, int line_width, bool bFill) {
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glOrtho(0.0, width(), height(), 0.0, -1.0, 1.0);
...
void CUVGLWidget::drawText(QPoint lb, const char* text, int fontsize, QColor clr) {
...
void CUVGLWidget::resizeGL(int w, int h) {
	glViewport(0, 0, w, h);
...
void CUVGLWidget::setVertices(double ratio) {
	GLfloat w = 1.0, h = 1.0;
	if (ratio < 1.0) {
		w = ratio;
	} else {
...
void CUVGLWidget::setVertices(QRect rc) {
	int wnd_w = width();
	int wnd_h = height();
	if (wnd_w <= 0 || wnd_h <= 0) {
		return;
...
void CUVGLWidget::drawYUV(CUVFrame* pFrame) {
